...
;;;###autoload
;;; Replace the whole ua/gpt-rewrite with this:
(defun ua/gpt-rewrite (instruction)
...
(defvar ua/gpt-last-patch-file nil
  "Path to the last patch written by ua/gpt-patch-apply.")
...
(defun ua/gpt-patch-apply (prompt)
...
No preview buffer; uses git apply with safety checks."
  ...
  (let* ((root (file-name-as-directory (ua/project-root)))
         (sys (format
    ...
    (gptel-request
     ...
     (lambda (resp info)
       ...
       (let ((patch-file (make-temp-file "gpt5-" nil ".patch")))
         (setq ua/gpt-last-patch-file patch-file)
...
(defun ua/gpt-patch-preview (prompt)
     ...
     (lambda (resp info)
       ...
       (let ((buf (get-buffer-create "*GPT Patch Preview*")))
         ...
         (message "Preview only. To apply: git apply %s (last saved)."
                  (or ua/gpt-last-patch-file "the saved patch")))))))


;; -- 3) Magit helpers: safe branch / commit / push ------------------
;;;###autoload
(defun ua/git--safe-branch-name (prefix)
...
;;;###autoload
(defun ua/git-make-safe-branch (prefix)
...
API via ghub."
  ...
  (let* ((repo (forge-get-repository :tracked)))
    ...
    (let* ((owner  (oref repo owner))
      ...
      (unless branch (user-error "Not on a branch"))
      (ua/git-push-current) ;; ensure branch is on origin
...
;;;###autoload
(defun ua/gptel-probe-models (&optional models)
...
;;;###autoload
(defun ua/yas-expand-by-key (key &optional mode)
...
(defvar ua/gpt-compose-buffer-name "*GPT Compose*")
(defvar ua/gpt-compose-kind nil)  ;; 'patch or 'rewrite (buffer-local)
(put 'ua/gpt-compose-kind 'permanent-local t)

(define-minor-mode ua/gpt-compose-mode
  "Minor mode for composing GPT requests (patch/rewrite)."
  :lighter " GPT-Compose"
  :keymap (let ((m (make-sparse-keymap)))
            (define-key m (kbd "C-c C-c") #'ua/gpt-compose-submit)
            (define-key m (kbd "C-c C-k") #'ua/gpt-compose-cancel)
            m))

(defun ua/gpt--compose-pop (title kind initial)
  "Open a compose buffer titled TITLE for KIND with INITIAL text."
  (let* ((buf (get-buffer-create ua/gpt-compose-buffer-name)))
    (pop-to-buffer buf)
    (erase-buffer)
    (insert initial)
    (goto-char (point-min))
    (markdown-mode)               ;; or gfm-mode, if you prefer
    (ua/gpt-compose-mode 1)
    (setq-local ua/gpt-compose-kind kind)
...
(defun ua/gpt-compose-cancel ()
  (interactive)
  (kill-buffer ua/gpt-compose-buffer-name)
  (message "Canceled."))

(defun ua/gpt-compose-submit ()
  ...
  (let* ((prompt (buffer-substring-no-properties (point-min) (point-max)))
         (kind ua/gpt-compose-kind))
    (unless (and prompt (> (length (string-trim prompt)) 0))
      (user-error "Prompt is empty."))
    (pcase kind
      ('patch   (ua/gpt-patch-apply prompt))   ;; uses your auto-apply flow
      ('rewrite (ua/gpt-rewrite prompt))       ;; call your rewrite function
...
;; ---------- Top authors (commits) ----------
(defun ua/git-top-authors (&optional since branch limit)
...
(defun ua/git-bus-factor (&optional since branch threshold)
...
(defun ua/git-file-owners (&optional file)
...
(with-eval-after-load 'magit
  (define-key magit-status-mode-map (kbd "C-c s a") #'ua/git-top-authors)
  (define-key magit-status-mode-map (kbd "C-c s b") #'ua/git-bus-factor)
  (define-key magit-status-mode-map (kbd "C-c s f") #'ua/git-file-owners))


(defun ua/gptel-use (m)
  (interactive)
  (setq gptel-model m)
...
(global-set-key (kbd "C-c g 5") (lambda () (interactive) (ua/gptel-use 'gpt-5)))
(global-set-key (kbd "C-c g o") (lambda () (interactive) (ua/gptel-use 'gpt-4o)))
(global-set-key (kbd "C-c g m") (lambda () (interactive) (ua/gptel-use 'gpt-4o-mini)))


;; -- Core keybindings (you can change these) ----------------------------

(global-set-key (kbd "C-c g r") #'ua/gpt-rewrite)
(global-set-key (kbd "C-c g b") #'ua/git-make-safe-branch)
(global-set-key (kbd "C-c g c") #'ua/git-commit-all)
(global-set-key (kbd "C-c g u") #'ua/git-push-current)
(global-set-key (kbd "C-c g R") #'ua/forge-open-pr)
(global-set-key (kbd "C-c g p") #'ua/gpt-patch-apply)   ;; auto-apply, no buffer overwrite
(global-set-key (kbd "C-c g P") #'ua/gpt-patch-preview) ;; preview-only when you want it
;; Keybindings: use composer instead of minibuffer for new requests
(global-set-key (kbd "C-c g C") #'ua/gpt-compose-patch)   ;; compose PATCH
(global-set-key (kbd "C-c g W") #'ua/gpt-compose-rewrite) ;; compose REWRITE
